...
function PulseInjetarEstiloPlanosOriginais(){
 if (document.getElementById('pulse-planos-originais-style')){return;}
 var estilo = document.createElement('style');
 estilo.id = 'pulse-planos-originais-style';
 estilo.textContent = '.pulse-planos-shell-original{position:absolute;left:50%;transform:translateX(-50%);width:min(calc(100vw - 40px),1760px);z-index:960;display:grid;grid-template-columns:46px minmax(0,1fr) 46px;gap:12px;align-items:center}.pulse-planos-viewport-original{overflow:hidden;padding:8px 0 16px}.pulse-planos-track-original{display:flex;gap:18px;overflow-x:auto;scroll-behavior:smooth;scroll-snap-type:x proximity;padding:8px 0 16px;-ms-overflow-style:none;scrollbar-width:none}.pulse-planos-track-original::-webkit-scrollbar{display:none}.pulse-planos-track-original.pulse-planos-fit{justify-content:space-between}.pulse-plano-grupo-original{position:relative;flex:0 0 auto;scroll-snap-align:start}.pulse-plano-grupo-original .gpc-e{position:absolute!important;margin:0!important}.pulse-planos-nav-original{width:46px;height:46px;border:0;border-radius:999px;background:#173fa3;color:#fff;font-size:24px;line-height:1;display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 10px 20px rgba(18,52,133,.18);transition:transform .2s ease,opacity .2s ease,background-color .2s ease}.pulse-planos-nav-original:hover{transform:translateY(-2px);background:#10358a}.pulse-planos-nav-original:disabled{opacity:.35;cursor:default;transform:none}.pulse-planos-note-original{grid-column:2;text-align:center;font-size:13px;line-height:1.45;color:#4d4d4d;padding:0 20px}@media all and (max-width:1500px){.pulse-planos-shell-original{width:min(calc(100vw - 24px),1560px)}}@media all and (max-width:1200px){.pulse-planos-shell-original{width:min(calc(100vw - 24px),1220px)}}@media all and (max-width:800px){.pulse-planos-shell-original{display:none!important}}';
 document.head.appendChild(estilo);
}
function PulseAtualizarNavegacaoPlanosOriginais(shell){
 if (!shell){return;}
 var trilha = shell.querySelector('.pulse-planos-track-original');
 var anterior = shell.querySelector('[data-pulse-planos-original=\"prev\"]');
 var proximo = shell.querySelector('[data-pulse-planos-original=\"next\"]');
 if (!trilha || !anterior || !proximo){return;}
 var temOverflow = trilha.scrollWidth > (trilha.clientWidth + 8);
 trilha.classList.toggle('pulse-planos-fit', !temOverflow);
 anterior.style.visibility = temOverflow ? 'visible' : 'hidden';
 proximo.style.visibility = temOverflow ? 'visible' : 'hidden';
 anterior.disabled = !temOverflow || trilha.scrollLeft <= 8;
 proximo.disabled = !temOverflow || (trilha.scrollLeft + trilha.clientWidth) >= (trilha.scrollWidth - 8);
}
function PulseCopiarEstilosComputadosPlano(origem, destino){
 if (!origem || !destino){return;}
 var estilos = window.getComputedStyle(origem);
 for (var i = 0; i < estilos.length; i++){
  var propriedade = estilos[i];
  destino.style.setProperty(propriedade, estilos.getPropertyValue(propriedade), estilos.getPropertyPriority(propriedade));
 }
}
function PulseCopiarEstilosArvorePlano(origem, destino){
 if (!origem || !destino){return;}
 PulseCopiarEstilosComputadosPlano(origem, destino);
 var filhosOrigem = origem.children || [];
 var filhosDestino = destino.children || [];
 for (var i = 0; i < filhosDestino.length; i++){
  if (filhosOrigem[i]){
   PulseCopiarEstilosArvorePlano(filhosOrigem[i], filhosDestino[i]);
  }
 }
}
function PulseCalcularLimitesPlanoOriginal(elementos){
 if (!elementos || !elementos.length){return null;}
 var esquerda = Number.POSITIVE_INFINITY;
 var topo = Number.POSITIVE_INFINITY;
 var direita = Number.NEGATIVE_INFINITY;
 var baixo = Number.NEGATIVE_INFINITY;
 elementos.forEach(function(elemento){
  var rect = elemento.getBoundingClientRect();
  esquerda = Math.min(esquerda, rect.left);
  topo = Math.min(topo, rect.top);
  direita = Math.max(direita, rect.right);
  baixo = Math.max(baixo, rect.bottom);
 });
 return {
  left: esquerda,
  top: topo,
  width: Math.max(1, direita - esquerda),
  height: Math.max(1, baixo - topo)
 };
}
function PulseClonarElementoPlanoOriginal(elemento, limites){
 var rect = elemento.getBoundingClientRect();
 var clone = elemento.cloneNode(true);
 clone.dataset.pulsePlanClone = 'true';
 clone.removeAttribute('id');
 clone.querySelectorAll('[id]').forEach(function(no){
  no.removeAttribute('id');
 });
 PulseCopiarEstilosArvorePlano(elemento, clone);
 clone.style.position = 'absolute';
 clone.style.left = Math.round(rect.left - limites.left) + 'px';
 clone.style.top = Math.round(rect.top - limites.top) + 'px';
 ...
 if (link && /assinar/i.test(PulseLimparTexto(link.textContent))){
  link.setAttribute('href', '#');
...
function PulseConfigurarCarrosselPlanosOriginal(){
 PulseInjetarEstiloPlanosOriginais();
 var textosDesktop = Array.from(document.querySelectorAll('.gpc-e.esconder_mobile.dd:not([data-pulse-plan-clone=\"true\"]) .c.e_texto, .gpc-e.esconder_mobile.dd:not([data-pulse-plan-clone=\"true\"]) .c.e_titulo'));
 var titulo = textosDesktop.find(function(el){
  return /Escolha o melhor plano/i.test(PulseLimparTexto(el.textContent));
 });
 if (!titulo){return;}
 var descricao = textosDesktop.find(function(el){
  return /Planos perfeitos para voc/i.test(PulseLimparTexto(el.textContent));
 });
 var nota = textosDesktop.find(function(el){
  return /^\*Valores promocionais/i.test(PulseLimparTexto(el.textContent));
 });
 var tituloRoot = titulo.closest('.gpc-e');
 var descricaoRoot = descricao ? descricao.closest('.gpc-e') : null;
 var notaRoot = nota ? nota.closest('.gpc-e') : null;
 var container = titulo.closest('.centralizar');
 var bloco = titulo.closest('.gpc-b');
 if (!container || !bloco){return;}
 var elementosDesktop = Array.from(container.children).filter(function(el){
  return el.classList && el.classList.contains('gpc-e') && el.classList.contains('esconder_mobile') && el.classList.contains('dd') && el.dataset.pulsePlanClone !== 'true';
 });
 var shellAtual = container.querySelector('.pulse-planos-shell-original');
 if (shellAtual){
  shellAtual.remove();
 }
 elementosDesktop.forEach(function(el){
 ...
 var velocimetros = elementosDesktop.map(function(root){
  return {
   root: root,
   text: PulseLimparTexto(root.textContent),
   rect: root.getBoundingClientRect()
  };
 }).filter(function(item){
  return item.root !== tituloRoot &&
   item.root !== descricaoRoot &&
   item.root !== notaRoot &&
   item.rect.width > 4 &&
   item.rect.height > 4 &&
   /(?:\d+\s*Mega|1\s*Giga)/i.test(item.text);
 }).sort(function(a, b){
  return PulsePontuacaoPlano(a.text) - PulsePontuacaoPlano(b.text);
 }).slice(0, 5);
 if (velocimetros.length < 5){return;}
 var topoMinimo = Math.min.apply(null, velocimetros.map(function(item){ return item.rect.top; })) - 120;
 var baseMaxima = Math.max.apply(null, velocimetros.map(function(item){ return item.rect.top; })) + 430;
 var candidatos = elementosDesktop.map(function(root){
  return {
   root: root,
   text: PulseLimparTexto(root.textContent),
   rect: root.getBoundingClientRect()
  ...
  return item.root !== tituloRoot &&
   ...
   item.rect.width > 4 &&
   item.rect.height > 4 &&
   item.rect.top >= topoMinimo &&
   item.rect.bottom <= baseMaxima &&
   item.rect.width <= 360 &&
   item.rect.height <= 620;
 });
 var grupos = velocimetros.map(function(item){
  return {
   speed: item.text,
   score: PulsePontuacaoPlano(item.text),
 ...
 candidatos.forEach(function(item){
  ...
  var melhorDistancia = Number.MAX_SAFE_INTEGER;
  grupos.forEach(function(grupo){
   var distancia = Math.abs(centroX - grupo.centerX) + (Math.abs(centroY - grupo.centerY) * 1.25);
   if (distancia < melhorDistancia){
    melhorDistancia = distancia;
    melhorGrupo = grupo;
   }
  });
  if (melhorGrupo){melhorGrupo.items.push(item);}
 });
 grupos = grupos.map(function(grupo){
  grupo.items = grupo.items.sort(function(a, b){
   var zA = parseInt(window.getComputedStyle(a.root).zIndex || '0', 10);
   var zB = parseInt(window.getComputedStyle(b.root).zIndex || '0', 10);
  ...
  return grupo.items.length;
 });
 if (!grupos.length){return;}
 var elementosParaOcultar = grupos.reduce(function(acumulador, grupo){
  grupo.items.forEach(function(item){
   if (acumulador.indexOf(item.root) === -1){
    acumulador.push(item.root);
   }
  });
  return acumulador;
 }, []);
 if (notaRoot && elementosParaOcultar.indexOf(notaRoot) === -1){
  elementosParaOcultar.push(notaRoot);
 }
 elementosParaOcultar.forEach(function(el){
  el.dataset.pulsePlanHidden = 'true';
  el.style.opacity = '0';
 ...
 var tituloRect = tituloRoot.getBoundingClientRect();
 var descricaoRect = (descricaoRoot || tituloRoot).getBoundingClientRect();
 var containerRect = container.getBoundingClientRect();
 var topRelativo = Math.max((Math.max(tituloRect.bottom, descricaoRect.bottom) - containerRect.top) + 24, topoMinimo - containerRect.top);
 var shell = document.createElement('section');
 shell.className = 'pulse-planos-shell-original';
 shell.innerHTML = '<button type="button" class="pulse-planos-nav-original" data-pulse-planos-original="prev" aria-label="Planos anteriores">&#8249;</button><div class="pulse-planos-viewport-original"><div class="pulse-planos-track-original"></div></div><button type="button" class="pulse-planos-nav-original" data-pulse-planos-original="next" aria-label="Próximos planos">&#8250;</button><div class="pulse-planos-note-original"></div>';
 shell.style.top = Math.round(topRelativo) + 'px';
 var trilha = shell.querySelector('.pulse-planos-track-original');
 grupos.forEach(function(grupo){
  var limites = PulseCalcularLimitesPlanoOriginal(grupo.items.map(function(item){
   return item.root;
  }));
  if (!limites){return;}
  var card = document.createElement('div');
  card.className = 'pulse-plano-grupo-original';
  card.style.width = Math.round(limites.width) + 'px';
  card.style.height = Math.round(limites.height) + 'px';
  grupo.items.forEach(function(item){
   card.appendChild(PulseClonarElementoPlanoOriginal(item.root, limites));
  });
  trilha.appendChild(card);
 });
 var notaEl = shell.querySelector('.pulse-planos-note-original');
 if (nota){
  notaEl.textContent = PulseLimparTexto(nota.textContent);
 }else{
  notaEl.remove();
 }
 var navegar = function(direcao){
  var larguraCard = trilha.querySelector('.pulse-plano-grupo-original');
  var deslocamento = larguraCard ? (larguraCard.getBoundingClientRect().width + 18) : 320;
  trilha.scrollBy({left: direcao * deslocamento, behavior: 'smooth'});
 };
 shell.querySelector('[data-pulse-planos-original="prev"]').addEventListener('click', function(){
  navegar(-1);
 });
 shell.querySelector('[data-pulse-planos-original="next"]').addEventListener('click', function(){
  navegar(1);
 });
 trilha.addEventListener('scroll', function(){
  PulseAtualizarNavegacaoPlanosOriginais(shell);
 }, {passive:true});
 container.appendChild(shell);
 requestAnimationFrame(function(){
  var alturaTotal = Math.ceil(topRelativo + shell.offsetHeight + 20);
  container.style.height = alturaTotal + 'px';
  bloco.style.height = alturaTotal + 'px';
  bloco.style.minHeight = alturaTotal + 'px';
  PulseAtualizarNavegacaoPlanosOriginais(shell);
...
function PulseConfigurarInteracoes(){
 ...
 PulseConfigurarCarrosselPlanosOriginal();
}
